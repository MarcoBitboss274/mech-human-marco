<template>
    <div class="admin-view">
        <div class="admin-view__header">
            <h1 class="page__title">{{ building.name ?? t('Struttura') }}</h1>
        </div>
                ...
                <template #details>
                    <div class="buildings-show__details">
                        <div class="buildings-show__details-header">
                            <BbButton v-if="can('buildings.edit')" @click="openBuildingModal">
                                {{ t('Modifica') }}
                            </BbButton>
                        </div>
                        <div class="buildings-show__details-grid">
                            ...
                            <div class="buildings-show__details-item">
                                <span class="buildings-show__label">{{ t('Agente') }}</span>
                                <span class="buildings-show__value">{{ building.agent_full_name ?? '--' }}</span>
                            </div>
                            <div class="buildings-show__details-item">
                ...
                <template #users>
                    <div class="buildings-show__users">
                        <div class="buildings-show__users-header">
                            <BbButton v-if="can('buildings.members.invite')" append:icon="plus" @click="openInviteModal">
                                {{ t('Invita membro') }}
                            </BbButton>
                        </div>
                    </div>
                    <BbTable
                        ...
                        item-value="id"
                        :items="users"
                        :actions="can('buildings.members.edit') || can('buildings.members.delete')"
                    >
                        <template #no-data>{{ t('Nessun utente associato a questa struttura') }}</template>
                        <template #building_role="{ item }">
                            <BuildingUserRoleBadge :role="item.pivot?.role" />
                        </template>
                ...
                <template #addresses>
                    <div class="buildings-show__addresses">
                        <div class="buildings-show__addresses-header">
                            <BbButton append:icon="plus" @click="openAddressModal(null)">
...
import AddressForm from '@/pages/buildings/partials/AddressForm.vue';
...
defineOptions({
    layout: (h: any, page: any) => h(AppLayout, { title: 'Dettaglio struttura' }, () => [page]),
});
...
const openBuildingModal = () => {
    selectedBuilding.value = props.building as Building;
    buildingModal.value = true;
};

const setDefaultAddress = (id: number) => {
    router.post(
        route('addresses.set-default', { address: id }),
        {},
        {
            onSuccess: () => {
