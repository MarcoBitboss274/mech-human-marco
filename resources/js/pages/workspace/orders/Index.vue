<template>
    <div class="workspace-view">
        <div class="workspace-view__header">
            <div>
                <h1 class="workspace-view__title">{{ t('Ordini') }}</h1>
            </div>
        </div>
        ...
        <div class="mb-6">
            <ul class="space-y-4">
                <BbTable :columns="columns" item-value="id" :items="orders.data ?? []" :loading="loading" actions>
...
defineOptions({
    layout: (h: any, page: any) => h(WorkspaceLayout, { title: 'Ordini' }, () => [page]),
});
...
const indexRoute = route('workspace.orders.index', { building: workspace.value?.slug });
const { loading, page, filters, resetFilters } = useIndexPage(indexRoute, filtersDefault, props.orders);
...
const openShow = (id: Order['id']) => {
    router.get(
        route('workspace.orders.show', {
