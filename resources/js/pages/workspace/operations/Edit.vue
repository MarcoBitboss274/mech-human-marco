...
defineOptions({
    layout: (h: any, page: any) => {
        ...
        return h(WorkspaceLayout, { title }, () => [page]);
    },
});
...
const { workspaceRole, currentUser } = usePermissions();

const isWorkspaceMember = computed(() => {
    const role = workspaceRole.value as any;
...
const resolveInitialForm = (): OperationCreateWizardForm => {
    ...
    const workspaceBuildingId = workspace.value?.id ?? null;

    if (!initial) {
        const base = {
            ...defaultForm(),
            building_id: workspaceBuildingId,
        };

        if (isWorkspaceMember.value) {
            base.user_id = currentUser.value?.id ?? null;
        }
...
buildingLabels.value = Object.fromEntries(props.wizard.buildings.map((building) => [building.id, building.name ?? String(building.id)]));
...
const { execute } = useAsyncFn(
    async () => {
        ...
        const targetRoute = isEditMode.value
            ...
            : route('workspace.operations.store-wizard', { building: workspace.value?.slug });
...
watch(
    ...
    async (buildingId, previousBuildingId) => {
        if (previousBuildingId !== undefined && buildingId !== previousBuildingId) {
            if (isWorkspaceMember.value && !isEditMode.value) {
        ...
        if (isWorkspaceMember.value) {
            const id = form.user_id ?? currentUser.value?.id ?? null;
            if (id && !userLabels.value[id]) {
                userLabels.value = {
                    [id]: currentUser.value?.name ? currentUser.value.name : String(id),
                    ...userLabels.value,
                };
            }
        }
