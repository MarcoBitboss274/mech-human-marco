<template>
    <div class="layout-sidebar" :class="{ 'layout-sidebar--expanded': open }">
        ...
        <div class="layout-sidebar__content" @click="onContentClick" v-if="userMeta.customer_buildings_count ?? 0 > 1">
...
const items = computed<Item[]>(() => {
    ...
    items.push({
        ...
        can: true,
        active: usePage().url.startsWith('/workspace/dashboard'),
    });
    if (workspace.value) {
        items.push({
            key: 'building',
            text: t('Struttura'),
            icon: 'building',
            href: route('workspace.building.index', { building: workspace.value?.slug }),
            can: true,
            active: usePage().url.startsWith('/workspace/' + workspace.value?.slug + '/building'),
        });
        items.push({
            key: 'operations',
            text: t('Lavorazioni'),
            icon: 'circle-stack',
            href: route('workspace.operations.index', { building: workspace.value?.slug }),
            can: true,
            active: usePage().url.startsWith('/workspace/' + workspace.value?.slug + '/operations'),
        });
        items.push({
            key: 'prescriptions',
            text: t('Prescrizioni'),
            icon: 'bandage',
            href: route('workspace.prescriptions.index', { building: workspace.value?.slug }),
            can: true,
            active: usePage().url.startsWith('/workspace/' + workspace.value?.slug + '/prescriptions'),
        });
        items.push({
            key: 'quotes',
            text: t('Preventivi'),
            icon: 'quote',
            href: route('workspace.quotes.index', { building: workspace.value?.slug }),
            can: true,
            active: usePage().url.startsWith('/workspace/' + workspace.value?.slug + '/quotes'),
        });
        // items.push({
        //     key: 'orders',
        //     text: t('Ordini'),
        //     icon: 'order',
        //     href: route('workspace.orders.index', { building: workspace.value?.slug }),
        //     can: true,
        //     active: usePage().url.startsWith('/workspace/' + workspace.value?.slug + '/orders'),
        // });
        items.push({
            key: 'invoices',
            text: t('Fatture'),
            icon: 'wallet',
            href: route('workspace.invoices.index', { building: workspace.value?.slug }),
            can: true,
            active: usePage().url.startsWith('/workspace/' + workspace.value?.slug + '/invoices'),
        });
    }
    return items;
});
